...
Renting a small warehouse will solve your problem.""")
    ...
    if status == 'owner':
        ...
        message = await msg.answer(f'Hello dear {msg.from_user.first_name}')
        await msg.answer('Main menu', reply_markup=m.client_start_markup)


# ЧТО ЭТО ЗА БЛОК ? СМ. ВЫШЕ
...
async def delete_previous_messages():
    global previous_messages, chat_id
...
async def output_order_attributes(cb: types.CallbackQuery, state: FSMContext):
    orders = await sync_to_async(funcs.get_orders)(cb.from_user.id)
...
async def forward_message(msg: types.Message, state: FSMContext):
    ...
    # await message_for_owner(cl_id['client_id'])
    # await msg.answer('exit', reply_markup=m.exit_owner)
    # dp.callback_query_handler(Text(['msg_for_owner']), state='*')  # вызвать для ответа овнеру - не работает
    await state.finish()
